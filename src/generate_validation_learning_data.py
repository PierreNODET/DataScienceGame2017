...
def generate_learning_validation(df):
    ...
    user_occurence = df[["user_id","ts_listen"]].groupby("user_id").count()
    user_occurence.rename(columns={'ts_listen':'occurence'},inplace=True)
    ...
    validation_rows = selected_rows.groupby("user_id").apply(lambda x: x.sample(n=1)).reset_index(level=1,drop=True)
...
learning_rows.to_csv("data/processed/learning.csv")
validation_rows.to_csv("data/processed/validation.csv")
